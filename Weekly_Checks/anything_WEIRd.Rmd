...
#set wd
wd = "S:/WatershedScience/Stream_Program/00_Methods/03_Loggernet/WeirDataWorkflow" # workflow folder on  sdrive
setwd(wd)
```
...
#110
if(sum(grepl("110", files))){
WS_110 <- paste0(dbfold,files[grepl("110", files)])
bind110 <- bind_rows(lapply(WS_110, format_head))%>% mutate(SiteName = "W110") %>% 
  relocate(SiteName)%>%
  filter(TIMESTAMP>= now("EST")-timerange)
bindlist <- list.append(bindlist,bind110)
}
...
files <- list.files(dbfold,ignore.case = T, pattern = "stage")[c(-2)]
bindlist <- list()
timerange <- days(7)
#files by site
#101
