...
for (i in 4:(length(plotnames))){
  ...
  plot <- temp %>%
    ...
    facet_wrap(vars(SiteName), scales = "free")+
    ylim(ifelse(min(temp[,3], na.rm = TRUE)<=-900000, -5, min(temp[,3], na.rm = TRUE)- 5), 
		max(temp[,3], na.rm = TRUE) + 5) +
...
files <- list.files(dbfold,ignore.case = F, pattern = "Stage")
files <- files[!grepl(files, pattern = "Samples")]
bindlist <- list()
timerange <- days(7)
#files by site
#101
